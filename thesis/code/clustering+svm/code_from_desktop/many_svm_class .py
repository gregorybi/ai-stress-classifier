...
Display 9 graphs for 9 SVM configs
...
models1 = [
   ...
   SVC(kernel="sigmoid", gamma="scale", C=C, decision_function_shape='ovo', probability=True),
   NuSVC(nu=0.2, probability=True),
   NuSVC(nu=0.8, kernel='sigmoid', probability=True),
   NuSVC(nu=0.3, kernel='poly', probability=True),
]
...
titles = [
    ...
    'NuSVM with '  + str(models1[6].get_params()['kernel']) + ' kernel, nu= ' + str(models1[6].get_params()['nu']) ,
...
for clf, title, ax, i in zip(models1, titles, subn.flatten(), range(len(titles))):
    if title == "LinearSVM (linear kernel)":
        ...
        # Compute ROC curve
        fpr, tpr, _ = roc_curve(y_test_ar, y_pred_proba[8:None])
